...
export function usePdfThumbnail({
  ...
  useEffect(() => {
    ...
    // Same underlying file: only swap when the new token meaningfully extends expiry.
    if (shouldSwapUrl(displayedThumbnailUrl, thumbnailUrl, false)) {
      setDisplayedThumbnailUrl(thumbnailUrl)
    }
  ...
  useEffect(() => {
    ...
    const renderPdf = async () => {
      try {
        ...
        if (pdfKey) previewCache.set(pdfKey, { dataUrl })
      } catch (err) {
        if (!controller.signal.aborted) {
          console.error('PDF render failed', err)
          setRenderedPreviewUrl(null)
          setImageError(true)
          if (pdfKey) previewCache.set(pdfKey, { error: true })
        }
      } finally {
